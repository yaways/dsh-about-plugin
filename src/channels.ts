...
import { failureOf } from './commands.ts'
import { DSH_NPM_PACKAGE } from './install.ts'
import type { CheckResult, IncomingCommit, NpmCheck, SourceCheck, VersionFacts } from './schemas.ts'
import { compareVersions } from './version.ts'

/** Official DeepSeek Harness origins the default allowlist pins. */
export const OFFICIAL_ORIGIN_ALLOWLIST: readonly string[] = [
...
async function readOrigin(run: CommandRunner, gitRoot: string): Promise<string | undefined> {
...
async function countRange(run: CommandRunner, gitRoot: string, range: string): Promise<number> {
...
/** Mutable builder backing one SourceCheck. */
 ...
 */
export async function checkSourceChannel(
  facts: VersionFacts,
  ...
  if (fetch) {
    ...
    if (!fetched.ok) return { ...state, channel: 'source', trackedRef: config.trackedRef, remoteRef, error: failureOf(fetched, 'git fetch failed') }
    state.fetched = true
  }
  const head = await run('git', ['rev-parse', 'HEAD'], { cwd: gitRoot })
  if (!head.ok) return { ...state, channel: 'source', trackedRef: config.trackedRef, remoteRef, error: failureOf(head, 'git rev-parse HEAD failed') }
  ...
  if (state.behind > 0) {
    const log = await run(
      'git',
      ['log', '--oneline', '--no-decorate', '-n', String(config.incomingLimit), `HEAD..${remoteRef}`],
      { cwd: gitRoot },
    )
    if (log.ok) {
      ...
      for (const line of log.stdout.split('\n')) {
        ...
        if (match !== null) incoming.push({ sha: match[1] ?? '', subject: match[2] ?? '' })
      }
...
/** Mutable builder backing one NpmCheck. */
interface NpmState {
  installed: string
  latest: string
  upToDate: boolean
 ...
 */
export async function checkNpmChannel(
  facts: VersionFacts,
  config: ChannelConfig,
  run: CommandRunner,
): Promise<NpmCheck> {
  const state: NpmState = { installed: facts.version, latest: '', upToDate: true }
...
export async function runCheck(
  ...
  const result: CheckResult = { checkedAt: Date.now(), form: facts.form, version: facts.version }
